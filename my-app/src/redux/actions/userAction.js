...
export const getUserInfo = createAsyncThunk("summoners", async (userNickName, { rejectWithValue }) => {
    ...
    try {
        const userInfoApi = await api.get(`kr.api.riotgames.com/lol/summoner/v4/summoners/by-name/${userNickName}?api_key=${API_KEY}`);
        const leagueInfoApi = await api.get(`kr.api.riotgames.com/lol/league/v4/entries/by-summoner/${userInfoApi.data.id}?api_key=${API_KEY}`);
        const matchInfoApi = await api.get(`asia.api.riotgames.com/lol/match/v5/matches/by-puuid/${userInfoApi.data.puuid}/ids?start=0&count=12&api_key=${API_KEY}`);

        const matchDetailPromises = matchInfoApi.data.map(async (matchId) => {
            const response = await api.get(`asia.api.riotgames.com/lol/match/v5/matches/${matchId}?api_key=${API_KEY}`);
